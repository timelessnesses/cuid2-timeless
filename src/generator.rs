...
use crate::{errors, utils::{self, SHAs}};
...
impl Default for Cuid {
    fn default() -> Self {
        ...
        let mut wrapper_rand: Box<dyn FnMut() -> f64> = Box::new(move || {
            randomity.gen()
        });
        Cuid {
            fingerprint: utils::create_fingerprint(&mut wrapper_rand, None, utils::SHAs::SHA3_512),
            random: wrapper_rand,
            counter: Box::new(utils::create_counter((randomed * INITIAL_COUNT_MAX as f64) as isize)),
...
impl Cuid {
    pub fn new(mut random: utils::RandomFunctionType, counter: utils::CounterFunctionType, length: usize, fingerprint: utils::FingerPrintFunctionType, sha_algo: SHAs) -> Self {
        Cuid {
    ...
    pub fn generate(&mut self, length: Option<usize>) -> Result<String, errors::Errors> {
        ...
        let base36_time = utils::base36_encode(std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
        let base36_count = utils::base36_encode((self.counter)() as u128);

        let salt = match utils::create_entropy(&mut self.random, Some(actual_length)).map_err(|_| {errors::Errors::LessThanOneEntropyError}) {
            Ok(s) => {
                s
            },
            Err(e) => {
        ...
        return Ok(first_letter.to_string() + &utils::create_hash(Some(hash_input), &self.sha_algo)[1..actual_length]);
    }
}